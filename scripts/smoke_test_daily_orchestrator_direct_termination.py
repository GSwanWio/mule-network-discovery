"""Validate direct termination when suppression exhausts the frontier."""
...
def forbidden_factory() -> object:
    raise AssertionError(
        "An exhausted frontier created an AI adapter."
    )
...
def main() -> None:
    with TemporaryDirectory() as directory:
        ...
        request = SourceLoadRequest.create(
            ...
            run_date=RUN_DATE,
            state_namespace=(
                "direct-termination-orchestrator-smoke"
            ),
        )
        ...
        counterparty_phase = run_counterparty_ai_phase(
            ...
            reset_state=True,
            adapter_factory=lambda: counterparty_adapter,
        )
        ...
        assert len(counterparty_adapter.calls) == 1
        ...
        print(
            "Daily orchestrator direct termination "
            "smoke test passed."
        )
        print("Provider load count: 1")
        print("Counterparty AI calls: 1")
        print("Customer payloads: 0")
        print("Recursive frontier steps: 0")
        print("Direct termination steps: 1")
